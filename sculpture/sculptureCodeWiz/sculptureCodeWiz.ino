...
const unsigned int outPort = 8000;       // Port for sending OSC messages
const unsigned int localPort = 7777;     // Local port to listen for OSC messages
...
// Pin definitions
// Touch Pins: D15, D13, D12, D14, D27, D33
const int touchPins[] = {T3, T4, T5, T6, T7, T8};
const char* pinNames[] = {                                                                                                                            "/wiz/1", "/wiz/2", "/wiz/3", "/wiz/4", "/wiz/5", "/wiz/6"};
...
void setup() {
  Serial.begin(115200);
  Serial.println("Setting up ESP32/Wiz");
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    sendOSCMessage("/wiz/distance", distance);
...
void sendOSCMessage(const char* address, int touchValue) {
  ...
  // Send to the original port (8000) TOUCH DESIGNER
  Udp.beginPacket(outIp, outPort);
  msg.send(Udp);
  Udp.endPacket();

  // Send to the new port (4444) PROCESSING
  Udp.beginPacket(outIp, 4444);
  msg.send(Udp);
  Udp.endPacket();
...
void oscDispatch(OSCMessage &msg) {
  ...
  if (strcmp(address, "/wiz/r") == 0) {
    oscRCallBack(msg);
  } else if (strcmp(address, "/wiz/g") == 0) {
    oscGCallBack(msg);
  } else if (strcmp(address, "/wiz/b") == 0) {
    oscBCallBack(msg);
  } else {
